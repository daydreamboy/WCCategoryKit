...
- (NSString *)valueOfUrlForKey:(NSString *)key usingConnector:(NSString *)connector usingSeparator:(NSString *)separator {
    ...
    NSUInteger locationOfSearchedKey = [self rangeOfString:searchedKey options:NSBackwardsSearch].location;
    if (locationOfSearchedKey == NSNotFound) {
        return nil;
    }
    else {
        NSUInteger locationOfSeparator = [self rangeOfString:separator options:0 range:NSMakeRange(locationOfSearchedKey, self.length - locationOfSearchedKey)].location;
        if (locationOfSeparator == NSNotFound) {
            locationOfSeparator = self.length;
        }
        
        NSUInteger start = locationOfSearchedKey + searchedKey.length;
        NSUInteger end = locationOfSeparator;
        NSString *value = [self substringWithRange:NSMakeRange(start, end - start)];
